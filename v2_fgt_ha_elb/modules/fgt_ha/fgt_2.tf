...
resource "azurerm_virtual_machine" "fgt_2" {
  ...
  storage_os_disk {
    name              = "${var.prefix}-osdisk-${var.fgt_2_id}-${random_string.random.result}"
    caching           = "ReadWrite"
  ...
  # Log data disks
  storage_data_disk {
    name              = "${var.prefix}-datadisk-${var.fgt_2_id}-${random_string.random.result}"
    managed_disk_type = "Standard_LRS"
    create_option     = "Empty"
    lun               = 0
    disk_size_gb      = "30"
  ...
  os_profile {
    computer_name  = "${var.prefix}-${var.fgt_2_id}"
    admin_username = var.admin_username
    admin_password = var.admin_password
    custom_data    = data.template_file.fgt_2_config.rendered
  }
...
// FGT config
data "template_file" "fgt_2_config" {
  ...
  vars = {
    ...
    license_file  = var.fgt_2_license_file

    public_port  = var.ports["public"]
    public_ip    = local.fgt_2_ni_ips["public"]
...
data "template_file" "fgt_2_fgcp-config" {
  ...
  vars = {
    ...
    mgmt_gw      = cidrhost(var.subnet_cidrs["mgmt"], 1)
    peerip       = local.fgt_1_ni_ips["mgmt"]
  }
}
...
resource "azurerm_public_ip" "fgt_2_ni_mgmt" {
  name                = local.fgt_2_ni_names["mgmt"]
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_network_interface" "fgt_2_ni_mgmt" {
  name                          = local.fgt_2_ni_names["mgmt"]
  location                      = var.location
  resource_group_name           = var.resource_group_name
  ...
  ip_configuration {
    ...
    private_ip_address            = local.fgt_2_ni_ips["mgmt"]
    primary                       = true
    public_ip_address_id          = azurerm_public_ip.fgt_2_ni_mgmt.id
  }

  tags = var.tags
}
// Active FGT Network Interface MGMT
resource "azurerm_network_interface" "fgt_2_ni_public" {
  name                          = local.fgt_2_ni_names["public"]
  location                      = var.location
  resource_group_name           = var.resource_group_name
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = "Static"
    private_ip_address            = local.fgt_2_ni_ips["public"]
  }

  tags = var.tags
}
// Active FGT Network Interface Private
resource "azurerm_network_interface" "fgt_2_ni_private" {
  name                          = local.fgt_2_ni_names["private"]
  location                      = var.location
  resource_group_name           = var.resource_group_name
  enable_ip_forwarding          = true
  enable_accelerated_networking = var.accelerate
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = "Static"
    private_ip_address            = local.fgt_2_ni_ips["private"]
  }
...
resource "azurerm_network_interface_security_group_association" "fgt_2_ni_mgmt_nsg" {
  network_interface_id      = local.fgt_2_ni_ids["mgmt"]
  network_security_group_id = var.fgt_nsg_ids["mgmt"]
}
resource "azurerm_network_interface_security_group_association" "fgt_2_ni_public_nsg" {
  network_interface_id      = local.fgt_2_ni_ids["public"]
